function analyze_load_response_results_v2()
% =========================================================================
...
if isempty(CSV_FILENAME)
    files = dir('test1_load_response.csv');
    if isempty(files)
...
%% ---- Figure 6: TE vs Cycle — coloured by load level, faceted by pass ----
figure('Name','Fig 6 — TE vs Cycle', 'Position',[50 100 1100 480]);

for pi = 1:n_passes
    subplot(1, n_passes, pi);
    hold on; grid on;
    ...
    for li = 1:n_loads
        ...
        % Per-cycle mean TE within this pass × load combination
        cyc_unique = unique(cyc_m)';
        cyc_mean   = arrayfun(@(c) mean(te_m(cyc_m == c)), cyc_unique);

        plot(cyc_unique, cyc_mean, '-o', ...
             'Color',           load_cmap(li,:), ...
             'MarkerFaceColor', load_cmap(li,:), ...
             'MarkerSize',      4, ...
             'LineWidth',       1.2, ...
             'DisplayName',     sprintf('%d%% load', load_lvls(li)));
    end

    % Failure tier reference lines
    yline( TIER1_TE_DEG, 'y--', 'Tier 1 (+1°)', 'LineWidth', 1.0, ...
           'LabelHorizontalAlignment', 'left');
    yline(-TIER1_TE_DEG, 'y--', '',              'LineWidth', 1.0, ...
           'HandleVisibility', 'off');
    yline( TIER2_TE_DEG, 'r--', 'Tier 2 (+3°)', 'LineWidth', 1.0, ...
           'LabelHorizontalAlignment', 'left');
    yline(-TIER2_TE_DEG, 'r--', '',              'LineWidth', 1.0, ...
           'HandleVisibility', 'off');
    yline(BASELINE_MEAN_TE, 'b:', 'Baseline', 'LineWidth', 1.2);

    xlabel('Cycle Number');
    ylabel('Mean TE per Cycle [deg]');
    title(sprintf('Pass %d — TE vs Cycle', passes(pi)));
    legend('Location', 'best', 'NumColumns', 1, 'FontSize', 7);
    xlim([0, max(cycle_col) + 1]);
end

sgtitle('Test 1 — Transmission Error vs Cycle (per pass, coloured by load level)', ...
