function go_to_angle_emergencybtn()
...
%% ---- E-STOP FIGURE ----
fig = figure('Name', '>>> RUNNING | Click here then press space to E-STOP <<<', ...
             'Color', [0.1 0.7 0.1], ...
             'NumberTitle', 'off', ...
...
while true
    ...
    while true
        ...
        fprintf('Current: %d mA | Dinamixel: %.2f deg | Encoder: %.2f deg\n', curr, pos, encoder_angle);
...
function angle = read_encoder(encoder)
    ...
    % Flush all queued packets except the latest one
    while encoder.NumBytesAvailable > 0
        data = readline(encoder);
        val  = str2double(strtrim(data));
        if ~isnan(val)
            last_angle = val;   % keep overwriting — ends up with latest
        end
    end
